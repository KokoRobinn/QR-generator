...
func sign(n int) int {
	if n > 0 { return 1 } else if n < 0 { return 0 } else { return 0 }
}
...
//draws a rectangle from (xs, ys) to (xd, yd)(inclusive), horizontally first, from index 0 in ´values´
func drawRect(code [][]uint8, values []uint8, xs, ys, xd, yd int) { 
	...
	l := len(values)
	draws := 0
	for x := xs; x != xd || !x_done ; x += x_dir {
		y_done = false
		for y := ys; y != yd || (!y_done && ys == yd) ; y += y_dir {
			fmt.Fprint(os.Stdout, "drawRect drawing at: ", x, " ", y, "; stops at: ", xd, " ", yd, "; moving in direction: ", x_dir, " ", y_dir, "\n")
...
func drawTimingLines(code [][]uint8, width int) {
	drawRect(code, []uint8{0b11, 0b10}[:], 8, 6, width - 8, 6)
	drawRect(code, []uint8{0b11, 0b10}[:], 6, 8, 6, width - 8)
	return
}
...
func makeFormatString(ecc ECC, mask int) []uint8 {
	...
	for bit_index := 14; bit_index > 0; bit_index-- {
...
func drawFormatString(code [][]uint8, ecc ECC, mask, end int) {
	...
	drawRect(code, format[0:6], 8, 0, 8, 5) //right of top-left orientationsquare
	drawRect(code, format[6:8], 8, 7, 8, 8) //corner of top-left orientationsquare
	code[7][8] = format[8] //corner of top-left orientationsquare
	drawRect(code, format[9: ], 5, 8, 0, 8) // bottom of top-left orientationsquare
	drawRect(code, format[0:8], end, 8, end - 7, 8) // bottom of top-right orientationsquare
	drawRect(code, format[8: ], 8, end, 8, end - 6) // right of bottom-left orientationsquare
	code[8][end - 7] = uint8(0b11) //dark module
}

func makeQRv3(text string) string {
	const width int = 29
	const end int = 28
	var code [][]uint8
	...
	drawRect(code, []uint8{0b10}[:], width - 8, 0, end, 8)
	drawRect(code, []uint8{0b10}[:], 0, width - 8, 8, end)
	
	drawOrientationSquare(code, 0, 0)
	drawOrientationSquare(code, width - 7, 0)
	drawOrientationSquare(code, 0, width - 7)

	drawTimingLines(code, width)
